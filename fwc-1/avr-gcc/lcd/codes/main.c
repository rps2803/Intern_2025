...
/* while(1)
 {
 LCD_Clear();
 LCD_Message("Hello");  // show counter 
_delay_ms(1000);     // set animation speed
}
}
 ...
 int n=0;
while (1) {
        LCD_Clear();         // clear previous number
        LCD_Integer(n);      // display current number
        _delay_ms(1000);     // delay for visibility
        n = (n + 1) % 10;    // increment and wrap back to 0 after 9
    }
}
*/
...
while (1) {
    ...
    LCD_Cmd(0x80);          // Move cursor to first row, first position
    LCD_Message("KEERTHANA MANDA");

    LCD_Cmd(0xC0);          // Move cursor to second row, first position
    LCD_Integer(n);
